...
def parse_args():
    ...
    parser.add_argument("--inform_data_file", type=str,default='dataset/inform/cityscapes_inform.pkl', help="path to inform_data_file")
...
def paddle_inference(args):
    ...
    for img_path in img_list:
        ...
        output_data = np.asarray(np.argmax(output_data, axis=2), dtype=np.uint8)
        save_predict(output_data, None, os.path.basename(img_path), 'cityscapes', args.save_path,
                     output_grey=False, output_color=True, gt_color=False)
        if args.benchmark:
            autolog.times.stamp()
            autolog.times.end(stamp=True)
            print('{}\t{}'.format(img_path,json.dumps([float(output_data.max()),float(output_data.min()),float(output_data.mean())])))
